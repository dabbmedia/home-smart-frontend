...
typedef struct _CustomData {
  GstElement *pipeline, *video_queue, *v4l2_source, *filter, *video_convert, *h264_encode, *h264_parse, *mpegts_mux, *tee, *hls_sink;
...
int main(int argc, char *argv[]) {
  CustomData data;
  GstPad *tee_video_pad, *tee_app_pad; //*tee_audio_pad,
  GstPad *queue_video_pad, *queue_app_pad; //*queue_audio_pad,
//  GstAudioInfo info;
//  GstCaps *audio_caps;
  GstBus *bus;
...
//  gst_caps_unref (audio_caps);
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.v4l2_source, data.filter, data.tee, data.video_queue, data.video_convert, data.h264_encode, data.h264_parse, data.mpegts_mux, data.hls_sink, data.app_queue,
      data.app_sink, NULL);
